...
results_path = os.path.expanduser('~/mdp_transition_sampling/results/benchmarking_w1_calc/')
# results_path = '/opt/project/results/benchmarking_w1_calc/'
...
lowest_ns = MWNSDistribution(seed, state_space=state_space, action_space=action_space, current_task=init_task)
lowest_ns.sensor_dist.set_degradation_params(bias_prob=2e-4, error_prob=2e-4, bias_max_delta=0.001, error_max_delta=0.001)
lowest_ns.actuator_dist.set_degradation_params(bias_prob=2e-4, error_prob=2e-4, bias_max_delta=0.001, error_max_delta=0.001)
lowest_ns.generate_sequence(sequence_length=sequence_length, append=False)
indices = get_mdp_change_indices(dist=lowest_ns)

w1s = calc_w1_of_nonstat_dist(dist=lowest_ns, n_states=50, n_transitions=5, n_reps=1)
w1s.to_csv(os.path.join(results_path, 'lowest_w1s_' + str(run_index) + '.csv'))
