...
from src.mask_lrl.network.network_utils import BaseNet
from src.mask_lrl.network.network_bodies import DummyBody, MultitaskMaskLinear
from src.mask_lrl.network.network_utils import layer_init

class VanillaNet(torch.nn.Module, BaseNet):
    def __init__(self, output_dim, body):
        super(VanillaNet, self).__init__()
        self.fc_head = layer_init(torch.nn.Linear(body.feature_dim, output_dim))
        self.body = body
        self.to(Config.DEVICE)

    def predict(self, x, to_numpy=False):
        phi = self.body(torch.Tensor(x))
        y = self.fc_head(phi)
        if to_numpy:
            y = y.cpu().detach().numpy()
        return y

class VanillaNet_CL(torch.nn.Module, BaseNet):
    ...
    def predict(self, x, task_label=None, to_numpy=False):
        x = torch.Tensor(x)
        task_label = torch.Tensor(task_label)
...
class DuelingNet(torch.nn.Module, BaseNet):
    def __init__(self, action_dim, body):
        super(DuelingNet, self).__init__()
        self.fc_value = layer_init(torch.nn.Linear(body.feature_dim, 1))
        self.fc_advantage = layer_init(torch.nn.Linear(body.feature_dim, action_dim))
        self.body = body
        self.to(Config.DEVICE)
...
class DuelingNet_CL(torch.nn.Module, BaseNet):
...
class CategoricalNet(torch.nn.Module, BaseNet):
    def __init__(self, action_dim, num_atoms, body):
        super(CategoricalNet, self).__init__()
        self.fc_categorical = layer_init(torch.nn.Linear(body.feature_dim, action_dim * num_atoms))
    ...
    def predict(self, x, to_numpy=False):
        ...
        prob = torch.nn.functional.softmax(pre_prob, dim=-1)
        if to_numpy:
            return prob.cpu().detach().numpy()
        return prob

class QuantileNet(torch.nn.Module, BaseNet):
    def __init__(self, action_dim, num_quantiles, body):
        super(QuantileNet, self).__init__()
        self.fc_quantiles = layer_init(torch.nn.Linear(body.feature_dim, action_dim * num_quantiles))
        self.action_dim = action_dim
...
class OptionCriticNet(torch.nn.Module, BaseNet):
    def __init__(self, body, action_dim, num_options):
        super(OptionCriticNet, self).__init__()
        self.fc_q = layer_init(torch.nn.Linear(body.feature_dim, num_options))
        self.fc_pi = layer_init(torch.nn.Linear(body.feature_dim, num_options * action_dim))
        self.fc_beta = layer_init(torch.nn.Linear(body.feature_dim, num_options))
    ...
    def predict(self, x):
        phi = self.body(torch.Tensor(x))
        q = self.fc_q(phi)
        beta = torch.nn.functional.sigmoid(self.fc_beta(phi))
        pi = self.fc_pi(phi)
        pi = pi.view(-1, self.num_options, self.action_dim)
        log_pi = torch.nn.functional.log_softmax(pi, dim=-1)
...
class ActorCriticNet(torch.nn.Module):
    def __init__(self, state_dim, action_dim, phi_body, actor_body, critic_body):
        ...
        self.fc_action = layer_init(torch.nn.Linear(actor_body.feature_dim, action_dim), 1e-3)
        self.fc_critic = layer_init(torch.nn.Linear(critic_body.feature_dim, 1), 1e-3)

        self.actor_params = list(self.actor_body.parameters()) + list(self.fc_action.parameters())
        self.critic_params = list(self.critic_body.parameters()) + list(self.fc_critic.parameters())
        self.phi_params = list(self.phi_body.parameters())

class ActorCriticNetSS(torch.nn.Module):
...
class DeterministicActorCriticNet(torch.nn.Module, BaseNet):
    def __init__(self,
                 state_dim,
                 action_dim,
                 actor_opt_fn,
    ...
    def actor(self, phi):
        return torch.nn.functional.tanh(self.network.fc_action(self.network.actor_body(phi)))

    def critic(self, phi, a):
        return self.network.fc_critic(self.network.critic_body(phi, a))

class GaussianActorCriticNet(torch.nn.Module, BaseNet):
    def __init__(self,
                 state_dim,
                 action_dim,
                 phi_body=None,
        ...
        self.network = ActorCriticNet(state_dim, action_dim, phi_body, actor_body, critic_body)
        self.std = torch.nn.Parameter(torch.ones(1, action_dim))
        self.to(Config.DEVICE)
...
class GaussianActorCriticNet_SS(torch.nn.Module, BaseNet):
...
class GaussianActorCriticNet_CL(torch.nn.Module, BaseNet):
    LOG_STD_MIN = -0.6931 #-20.
    LOG_STD_MAX = 0.4055 #1.3
    def __init__(self,
                 state_dim,
        ...
        self.network.fc_log_std = layer_init(torch.nn.Linear(self.network.actor_body.feature_dim, \
            action_dim), 1e-3)
        self.network.actor_params += [p for p in self.network.fc_log_std.parameters() if p.requires_grad is True]
        self.to(Config.DEVICE)
...
class CategoricalActorCriticNet(torch.nn.Module, BaseNet):
    def __init__(self,
                 state_dim,
                 action_dim,
                 phi_body=None,
...
# actor-critic net for continual learning where tasks are labelled using
# supermask superposition algorithm
class CategoricalActorCriticNet_SS(torch.nn.Module, BaseNet):
    def __init__(self,
                 state_dim,
                 action_dim,
                 task_label_dim=None,
...
# actor-critic net for continual learning where tasks are labelled
class CategoricalActorCriticNet_CL(torch.nn.Module, BaseNet):
    def __init__(self,
                 state_dim,
                 action_dim,
                 task_label_dim=None,
