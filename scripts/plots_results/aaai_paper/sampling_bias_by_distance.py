...
with open(os.path.join(project_path_local, 'scripts', 'local_experiments', 'simplegrid_dists_from_samples.pkl'), 'rb') as file:
    results_dict = pickle.load(file)
    true = np.array(results_dict['true'])
    sampled = np.array(results_dict['sampled'])

# # # # #
# Overall statistics:
# # # # #
np.mean(true - sampled)
np.std(true - sampled)

plt.scatter(true, true - sampled, s=3, alpha=0.25)
coef = np.polyfit(true,true - sampled,1)
...
scipy.stats.linregress(true, true-sampled)

# # # # #
# Binned plot:
# # # # #
plotdata = pd.DataFrame({'true': true, 'sampled': sampled})
...
for ind in range(len(bins) - 1):
    ...
    if this_boxplot_data.shape[0] > 0:
        plt.violinplot((this_boxplot_data.sampled - this_boxplot_data.true), positions=[position], showmedians=True, showextrema=False, widths=0.75, bw_method=0.125)
