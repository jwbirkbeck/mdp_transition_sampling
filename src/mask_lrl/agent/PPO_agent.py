...
# from src.mask_lrl.network import *
# from src.mask_lrl.component import *
# from ..mask_modules import *
from src.mask_lrl.agent.BaseAgent import BaseAgent, BaseContinualLearnerAgent
from src.mask_lrl.utils.misc import Batcher
from src.mask_lrl.component.replay import Replay
from src.mask_lrl.mask_modules.mmn.mask_nets import set_model_task, consolidate_mask, cache_masks, set_num_tasks_learned
...
class PPOAgent(BaseAgent):
    ...
    def iteration(self):
        ...
        advantages = torch.Tensor(np.zeros((config.num_workers, 1)))
...
class PPOContinualLearnerAgent(BaseContinualLearnerAgent):
    ...
    def iteration(self):
        ...
        for _ in range(config.optimization_epochs):
            ...
            while not batcher.end():
                ...
                self.opt.zero_grad()
                (policy_loss + value_loss).backward()
                norm_ = torch.nn.utils.clip_grad_norm_(self.network.parameters(), config.gradient_clip)
    ...
    def _rollout_normal(self, states, batch_task_label):
        ...
        rollout = []
        for _ in range(config.rollout_length):
            _, actions, log_probs, _, values, _ = self.network.predict(states, task_label=batch_task_label)
            next_states, rewards, terminals, _ = self.task.step(actions.cpu().detach().numpy())
            self.episode_rewards += rewards
            rewards = config.reward_normalizer(rewards)
            for i, terminal in enumerate(terminals):
    ...
    def _rollout_metaworld(self, states, batch_task_label):
        ...
        for _ in range(config.rollout_length):
            ...
            next_states, rewards, terminals, truncated, infos = self.task.step(actions.cpu().detach().numpy().reshape(-1))
            terminals = [terminals or truncated]
...
class LLAgent(PPOContinualLearnerAgent):
    ...
    def _label_to_idx(self, task_label):
        ...
        for task_idx, seen_task_label in self.seen_tasks.items():
            if np.linalg.norm((np.array(task_label) - np.array(seen_task_label)), ord=2) < eps:
